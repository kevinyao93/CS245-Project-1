...
After the comparison between Bert and SpaCy, we decided to choose the word embeddings provided by spaCy for the following work. Folder `expansion` contains the python codes for entity expansion using spaCy word embeddings. For SpaCy expansion, please install spacy package and download en_core_web_lg model by running `pip install spacy`, `python -m spacy download en_core_web_lg`.
